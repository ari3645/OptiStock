...
<form method="POST">
    ...
    <input type="text" name="nom_commande" required value="<?= htmlspecialchars($nom_commande) ?>"><br><br>
    ...
    <div style="display:flex; gap:40px;">
            ...
            <?php foreach ($lots_disponibles as $lot): ?>
                ...
                <?php if ($restant > 0): ?>
                    <div style="margin-bottom: 8px;">
                        <?= htmlspecialchars($lot['nom']) ?> (Stock: <?= $restant ?>)
                        <input type="number" name="quantite" min="1" max="<?= $restant ?>" value="1" required>
                        <input type="hidden" name="lot_id" value="<?= $lot['id'] ?>">
                        <button type="submit" name="action" value="ajouter_lot">Ajouter</button>
                    </div>
                <?php else: ?>
        ...
        <div style="width:40%;">
            ...
            <button type="submit" name="action" value="vider_commande" style="color:red;" onclick="return confirm('Vider la commande en cours ?');">Vider</button>

            <br>
            <button type="submit" name="action" value="creer_commande">Créer la commande</button>
        </div>
    </div>
</form>
</body>
